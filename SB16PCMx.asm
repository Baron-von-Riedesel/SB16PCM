...
;--- This is the 16-bit protected-mode variant.
...
fileread proc stdcall hFile:word, pBuffer:dword, dwSize:dword
	mov dx, word ptr pBuffer
	test dwChunks, 1
	jz @F
...
@@:
	...
	mov al, [bx+pgtab]	; al=page register
	mov ah, 0
	mov wDmaPageChn, ax
	movzx eax, bx
	and al, 3
...
;--- out: eax=sample buffer linear address
...
getsamplebuffer proc uses esi
	...
	jz @F					;if no, use the buffer
	shl eax, 16		; calculated difference to next 64 kb border
	push eax
	sub eax, esi
	mov wOffset, ax
	pop esi
...
;--- get sample buffer
	...
	.if bVerbose
		mov dx, wDosAddr
		invoke printf, CStr("sample buffer=%lX segm=%X",10), eax, dx
	.endif
...
;--- fill sample buffer
	...
	mov dx, wDosSel
	mov ax, wOffset
	invoke fileread, hFile, dx::ax, dwSize	; fill first half of buffer
	jc exit
	movzx eax, ax
	sub dwSize, eax
	mov dx, wDosSel
	mov ax, wOffset
	invoke fileread, hFile, dx::ax, dwSize	; fill second half of buffer
	jc exit
	movzx eax, ax
	sub dwSize, eax
...
;--- setup isr
	...
	mov ax, 204h
	int 31h
	mov word ptr [OldIntSB+0], dx
	mov word ptr [OldIntSB+2], cx
	mov cx, cs
...
@@:
	...
	mov dx, wDosSel
	mov ax, wOffset
	invoke fileread, hFile, dx::ax, dwSize
	jc exit
	movzx eax, ax
	sub dwSize, eax
	jmp waitloop
